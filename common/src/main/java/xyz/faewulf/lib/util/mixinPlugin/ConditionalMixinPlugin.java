        ...
        try {
            ...
            if (mixinClass.visibleAnnotations != null) {
                for (AnnotationNode node : mixinClass.visibleAnnotations) {
                    ...
                    if (configClass.isAnnotationPresent(ModConfig.class)) {
                        ModConfig modConfig = configClass.getAnnotation(ModConfig.class);
                        String MOD_ID = modConfig.mod_id();

                        if (MOD_ID != null) {
                            Config.preLoad(MOD_ID, configClass);
                        }
                    }
